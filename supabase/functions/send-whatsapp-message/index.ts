...
Deno.serve(async (req) => {
  ...
  try {
    ...
    try {
      ...
      console.log('[send-whatsapp-message] Evolution API raw response:', responseText);
      evolutionData = responseText ? JSON.parse(responseText) : null;
    } catch (parseError) {
      console.error('[send-whatsapp-message] Failed to parse Evolution API response:', parseError);
      evolutionData = null;
    }

    // Extract message ID from Evolution API response (handle various response formats)
    ...
    if (evolutionData) {
      ...
      } else if (evolutionData.messageId) {
        messageId = evolutionData.messageId;
      } else if (evolutionData.id) {
        messageId = evolutionData.id;
      } else if (evolutionData.message?.key?.id) {
        messageId = evolutionData.message.key.id;
      }
    }
    
    console.log('[send-whatsapp-message] Using message ID:', messageId);
